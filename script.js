...
function onCheckout(){
  if(Object.keys(cart).length === 0){ showToast('Necesitas agregar algo al carrito.', 'warn'); return; }
  // Simulación de compra
  showToast('Gracias por tu compra, mi estimad@. Total: ' + document.getElementById('total').textContent + ' C$', 'success');
  // reset
...
const dinoGame = {
  ...
  init() {
    this.ctx = this.canvas.getContext('2d');
    this.canvas.height = 300; this.canvas.width = 300;
